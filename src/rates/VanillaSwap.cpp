...
double VanillaSwap::floatingLegPV(const YieldCurve& discountCurve, const YieldCurve& forwardCurve) const {
    (void)forwardCurve;
    // Phase 1 deliberately uses a single-curve par-floater approximation.
    // The forward curve argument is kept so the interface can grow into multi-curve pricing later.
