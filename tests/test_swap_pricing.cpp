...
TEST_CASE("Receiver swap has positive DV01 near par") {
    const fi::rates::YieldCurve curve({1.0, 2.0, 5.0, 10.0}, {0.04, 0.04, 0.04, 0.04});
    const fi::rates::VanillaSwap templateSwap("S5Y", 1000000.0, 0.04, 5.0, 2, 4, false, 1.0);
    const double par = templateSwap.parRate(curve, curve);
    const fi::rates::VanillaSwap receiver("S5Y", 1000000.0, par, 5.0, 2, 4, false, 1.0);
    REQUIRE(receiver.dv01(curve, curve) > 0.0);
}
