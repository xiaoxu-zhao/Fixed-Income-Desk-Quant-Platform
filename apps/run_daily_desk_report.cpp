...
int main(int argc, char** argv) {
    try {
        ...
        const std::string hedge = largestRisk > 0.0
            ...
            {"metric", "value"},
            {
                {"pv_yesterday_usd", formatDouble(pvYesterday, 2)},
                {"pv_today_usd", formatDouble(pvToday, 2)},
                {"daily_pnl_usd", formatDouble(dailyPnl, 2)},
                {"total_dv01_usd_per_bp", formatDouble(totalDv01, 2)},
                {"largest_keyrate_bucket_years", formatDouble(largestKey, 0)},
                {"largest_keyrate_dv01_usd_per_bp", formatDouble(largestRisk, 2)},
        ...
        for (const auto& value : portfolio.instrumentValuations(yesterday, today)) {
            ...
            {"instrument_id", "type", "pv_yesterday_usd", "pv_today_usd", "pnl_usd",
             "dv01_usd_per_bp", "convexity_usd_per_rate_squared"},
            instrumentRows);
        ...
        for (const auto& [key, risk] : keyDv01) {
            ...
            {"key_maturity_years", "keyrate_dv01_usd_per_bp"},
            keyRows);
