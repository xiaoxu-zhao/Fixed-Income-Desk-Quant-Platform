...
def main() -> None:
    ...
    plt.figure(figsize=(10, 4))
    plt.plot(intraday["timestamp_ns"] / 1_000_000, intraday["spread_price"] * 64.0, color="#B07AA1")
    plt.xlabel("Time (ms)")
    ...
    selected = pd.DataFrame({
        "metric": ["slippage_ticks", "fill_ratio", "implementation_shortfall_usd", "execution_cost_vs_vwap_usd"],
        "value": [
            float(metrics.get("slippage_ticks", 0.0)),
            float(metrics.get("fill_ratio", 0.0)),
            float(metrics.get("implementation_shortfall_usd", 0.0)),
            float(metrics.get("execution_cost_vs_vwap_usd", 0.0)),
        ],
    })
