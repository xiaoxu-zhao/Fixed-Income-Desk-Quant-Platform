...
def main() -> None:
    ...
    colors = ["#59A14F" if x >= 0 else "#E15759" for x in instrument["pv_today_usd"]]
    plt.bar(instrument["instrument_id"], instrument["pv_today_usd"] / 1_000_000, color=colors)
    ...
    colors = ["#59A14F" if x >= 0 else "#E15759" for x in keyrate["keyrate_dv01_usd_per_bp"]]
    plt.bar(keyrate["key_maturity_years"].astype(str) + "Y", keyrate["keyrate_dv01_usd_per_bp"], color=colors)
