...
def main() -> None:
    ...
    daily_pnl = float(daily["daily_pnl_usd"])
    full_pnl = float(pnl["full_revaluation_pnl"])
    key_explained = float(pnl["keyrate_explained"])
    residual = float(pnl["residual"])
    largest_row = keyrate.iloc[keyrate["keyrate_dv01_usd_per_bp"].abs().idxmax()]
    largest_key = int(largest_row["key_maturity_years"])
    largest_dv01 = float(largest_row["keyrate_dv01_usd_per_bp"])
    ...
    max_intraday_loss = float(intraday["intraday_pnl_usd"].min())
    max_intraday_gain = float(intraday["intraday_pnl_usd"].max())
    close_intraday_pnl = float(intraday["intraday_pnl_usd"].iloc[-1])
    ...
    lines = [
        ...
        "",
        (
            f"The largest pillar-bump key-rate exposure is the {largest_key}Y bucket with DV01 of "
            f"{money(largest_dv01)} per bp. Total desk DV01 is "
            f"{daily.get('total_dv01_usd_per_bp', 'n/a')} per bp, and the suggested hedge is `{hedge}`."
        ),
        "",
        ...
        (
            ...
            "full revaluation move. Convexity and carry are deliberately simple approximations, "
            "so the unexplained bucket should be read as model residual rather than production attribution."
        ),
        "",
        ...
        "",
        (
            f"The stylized hedge execution filled {fill_ratio:.1%} of the requested quantity. "
